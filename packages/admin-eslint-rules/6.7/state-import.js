export default {
	meta: {
		...
		docs: {
			description:
				"Replace HeyFrame.State with HeyFrame.Store (and destructured State accordingly).",
			category: "Best Practices",
			recommended: false,
		},
		fixable: "code",
		schema: [],
		minHeyFrameVersion: "6.7.0.0",
	},
	...
	create(context) {
		...
		function _isHeyFrameState(node) {
			return (
				node &&
		...
		return {
			// Check for destructuring: const { State } = HeyFrame;
			VariableDeclarator(node) {
				if (
					node.init &&
					node.init.type === "Identifier" &&
					node.init.name === "HeyFrame" &&
						...
						computedText = props
							.map((prop) => {
								return (
									...
									baseIndent +
									"    " +
									`return HeyFrame.Store.get('${storeName}').${prop};\n` +
									baseIndent +
									"}" +
							...
							.map((prop) => {
								...
								if (
									...
									return (
										...
										baseIndent +
										"    " +
										`return HeyFrame.Store.get('${storeName}').${mappedProp};\n` +
										baseIndent +
										"},"
									);
								}
			...
			CallExpression(node) {
				if (
					...
					if (
						...
						stateMember.object.name === "HeyFrame" &&
					...
					if (
						...
						if (parts.length === 2) {
							...
							const argsText = args
								...
								.join(", ");
							const newCode = `${shortHand ? "" : "HeyFrame."}Store.get('${storeName}').${methodName}(${
								argsText ? argsText : ""
							})`;
							context.report({
								node,
								message:
									"Replace State.commit/HeyFrame.State.commit call with HeyFrame.Store.get call.",
								fix(fixer) {
