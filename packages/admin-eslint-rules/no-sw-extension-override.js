export default {
	meta: {
		...
		docs: {
			description:
				"Disallow overwriting sw-extension-* components using HeyFrame.Component.override",
	...
	create(context) {
		...
		return {
			VariableDeclarator(node) {
				...
				// const Component = HeyFrame.Component;
				if (
					node.id.type === "Identifier" &&
					node.init &&
					node.init.type === "MemberExpression" &&
					node.init.object.type === "Identifier" &&
					node.init.object.name === "HeyFrame" &&
			...
			CallExpression(node) {
				const isHeyFrameComponentOverride =
				...
				if (isHeyFrameComponentOverride || isAliasOverride) {
